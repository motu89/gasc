...
export default function AdminVendorsManagement() {
  ...
  const loadVendors = async () => {
    setLoading(true)
    try {
      const response = await apiRequest<{ vendors: Vendor[] }>('/api/admin/vendors')
  ...
  const handleDelete = async (vendorId: string) => {
    if (!confirm('Are you sure you want to delete this vendor? This will also remove access to their products. This action cannot be undone.')) {
      return
    }

    try {
      setDeletingId(vendorId)
      await apiRequest(`/api/admin/vendors?vendorId=${vendorId}`, { method: 'DELETE' })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="ml-0 min-h-screen p-4 pt-16 sm:p-6 md:p-8 lg:ml-64 lg:pt-8">
        <div className="mb-6 flex flex-col items-start justify-between gap-3 sm:mb-8 sm:flex-row sm:items-center sm:gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">Vendor Management</h1>
                <p className="mt-1 text-sm text-gray-600">Manage all marketplace vendors</p>
              </div>
            </div>
          </div>
          <button
        ...
        {loading ? (
          <div className="rounded-xl bg-white p-8 text-center text-sm text-gray-600 shadow-md">Loading vendors...</div>
        ) : vendors.length === 0 ? (
          <div className="rounded-xl bg-white p-8 text-center text-sm text-gray-600 shadow-md">No vendors found.</div>
        ) : (
                ...
                <tbody className="divide-y divide-gray-200">
                  {vendors.map((vendor) => (
                    <tr key={vendor.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900 sm:px-6 sm:py-4">{vendor.name}</td>
                      <td className="px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4">{vendor.email}</td>
                      <td className="hidden px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4 md:table-cell">{vendor.phone || 'N/A'}</td>
                      <td className="hidden max-w-xs truncate px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4 lg:table-cell" title={vendor.address}>{vendor.address || 'N/A'}</td>
                      <td className="hidden px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4 md:table-cell">{formatDate(vendor.createdAt)}</td>
