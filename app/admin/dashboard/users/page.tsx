...
export default function AdminUsersManagement() {
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      ...
      await apiRequest(`/api/admin/users?userId=${userId}`, { method: 'DELETE' })
  ...
  const getRoleBadge = (role: string) => {
    ...
    return (
      <span className={`rounded-full px-3 py-1 text-xs font-semibold ${styles[role] || 'bg-gray-200 text-gray-700'}`}>
        {labels[role] || role}
      </span>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="ml-0 min-h-screen p-4 pt-16 sm:p-6 md:p-8 lg:ml-64 lg:pt-8">
        ...
        {loading ? (
          <div className="rounded-xl bg-white p-8 text-center text-sm text-gray-600 shadow-md">Loading users...</div>
        ) : users.length === 0 ? (
          <div className="rounded-xl bg-white p-8 text-center text-sm text-gray-600 shadow-md">No users found.</div>
        ) : (
              ...
              <table className="w-full">
                <thead className="border-b border-gray-200 bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 sm:px-6 sm:py-4 sm:text-sm">Role</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {users.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900 sm:px-6 sm:py-4">{u.name}</td>
                      <td className="px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4">{u.email}</td>
                      <td className="px-4 py-3 sm:px-6 sm:py-4">{getRoleBadge(u.role)}</td>
                      <td className="hidden px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4 md:table-cell">{u.phone || 'N/A'}</td>
                      <td className="hidden max-w-xs truncate px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4 lg:table-cell" title={u.address}>{u.address || 'N/A'}</td>
                      <td className="hidden px-4 py-3 text-sm text-gray-600 sm:px-6 sm:py-4 md:table-cell">{formatDate(u.createdAt)}</td>
